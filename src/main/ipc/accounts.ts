...
export function registerAccountHandlers(win: BrowserWindow, view: WebContentsView): void {
    ...
    ipcMain.handle("accounts:login", async (_e, id: string): Promise<AutoLoginResult> => {
        ...
        try {
            ...
            try {
                const permData = await withTimeout(
                    apiGetAuth(`${apiBaseUrl}/mas/permission/${userData.role}`, userData.auth_token as string),
                    8_000,
                );
                if (permData && typeof permData === "object") {
                    userData.permissions = JSON.stringify(permData);
                }
            } catch { /* non-fatal — continua sem permissions */ }
...
function apiGetAuth(url: string, token: string): Promise<Record<string, unknown>> {
    return new Promise((resolve, reject) => {
        const { hostname, pathname, search } = new URL(url);
        https.get(
            { hostname, path: pathname + search, headers: { Accept: "application/json", "x-api-key": token } },
            (res) => {
                let raw = "";
                res.on("data", c => raw += c);
                res.on("end", () => { try { resolve(JSON.parse(raw)); } catch { reject(new Error("JSON parse error")); } });
